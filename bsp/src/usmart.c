...
/* 原版依赖 CubeMX 生成的 usart.h/tim.h/bsp_tick.h,这个项目里没有这几个文件:
   输出统一改用项目自己的 rt_kprintf()(不引入标准库 <stdio.h> 的 printf,
   避免跟 nano.specs/nosys.specs 下的库内实现打架),
   TIM4 句柄就地声明在下面,不需要单独的 tim.h。 */
...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 1://帮助指令
			rt_kprintf("\r\n");
#if USMART_USE_HELP 
			rt_kprintf("------------------------USMART V3.3------------------------ \r\n");
			rt_kprintf("USMART there are 7 system commands (must be lowercase):\r\n");
			rt_kprintf("?:      Get help information\r\n");
			rt_kprintf("help:   Get help information\r\n");
			rt_kprintf("list:   List of available functions\r\n\n");
			rt_kprintf("id:     List of IDs for available functions\r\n\n");
			rt_kprintf("hex:    Hex display,space+digit\r\n\n");
			rt_kprintf("dec:    Decimal display,space+digit\r\n\n");
			rt_kprintf("runtime:1,Enable function runtime timing; 0, turn off function runtime timing;\r\n\n");
			rt_kprintf("Please enter the function name and parameters according to the programming format and end with the Enter key key.\r\n");    
			rt_kprintf("--------------------------ALIENTEK------------------------- \r\n");
#else
			rt_kprintf("指令失效\r\n");
#endif
		...
		case 2://查询指令
			rt_kprintf("\r\n");
			rt_kprintf("-------------------------List Of Function--------------------------- \r\n");
			for(i=0;i<usmart_dev.fnum;i++)rt_kprintf("%s\r\n",usmart_dev.funs[i].name);
			rt_kprintf("\r\n");
			break;	 
		case 3://查询ID
			rt_kprintf("\r\n");
			rt_kprintf("-------------------------ID Of Function --------------------------- \r\n");
		...
		case 6://runtime指令,设置是否显示函数执行时间
			...
			if(i==0)//参数正常
			{
				...
				if(i==0)						   		//读取指定地址数据功能
				{
					if(USMART_ENTIMX_SCAN==0)rt_kprintf("\r\nError! \r\nTo EN RunTime function,Please set USMART_ENTIMX_SCAN = 1 first!\r\n");//报错
					else
					{
						usmart_dev.runtimeflag=res;
						if(usmart_dev.runtimeflag)rt_kprintf("Run Time Calculation ON\r\n");
						else rt_kprintf("Run Time Calculation OFF\r\n"); 
					}
...
void usmart_exe(void)
{
	...
	for(i=0;i<pnum;i++)//输出参数
	{
		...
		{
			rt_kprintf("%c",'"');			 
			rt_kprintf("%s",usmart_dev.parm+usmart_get_parmpos(i));
			rt_kprintf("%c",'"');
			temp[i]=(u32)&(usmart_dev.parm[usmart_get_parmpos(i)]);
		}else						  //参数是数字
		{
			temp[i]=*(u32*)(usmart_dev.parm+usmart_get_parmpos(i));
			if(usmart_dev.sptype==SP_TYPE_DEC)rt_kprintf("%ld",temp[i]);//10进制参数显示
			else rt_kprintf("0X%X",temp[i]);//16进制参数显示 	   
		}
		if(i!=pnum-1)rt_kprintf(",");
	}
	rt_kprintf(")");
#if USMART_ENTIMX_SCAN==1
	...
	if(rval==1)//需要返回值.
	{
		if(usmart_dev.sptype==SP_TYPE_DEC)rt_kprintf("=%lu;\r\n",res);//输出执行结果(10进制参数显示)
		else rt_kprintf("=0X%X;\r\n",res);//输出执行结果(16进制参数显示)	   
	}else rt_kprintf(";\r\n");		//不需要返回值,直接输出结束
	if(usmart_dev.runtimeflag)	//需要显示函数执行时间
	{ 
		rt_kprintf("Function Run Time:%d.%1dms\r\n",usmart_dev.runtime/10,usmart_dev.runtime%10);//打印函数执行时间 
	}	
}
//usmart扫描函数
...
//	if(USART_RX_STA&0x8000)//串口接收完成？
	if( g_tUART1_Handle.u16Uart_Rx_Sta & 0x8000 )
	{					   
//		len=USART_RX_STA&0x3fff;	//得到此次接收到的数据长度
		len = g_tUART1_Handle.u16Uart_Rx_Sta & 0x3FFF;
//		USART_RX_BUF[len]='\0';	//在末尾加入结束符. 
		g_tUART1_Handle.u8Uart_Hex_Buffer[len] = '\0';
//		sta=usmart_dev.cmd_rec(USART_RX_BUF);//得到函数各个信息
		sta = usmart_dev.cmd_rec(g_tUART1_Handle.u8Uart_Hex_Buffer);
		if(sta==0)usmart_dev.exe();	//执行函数 
		else 
		{  
//			len=usmart_sys_cmd_exe(USART_RX_BUF);
			len = usmart_sys_cmd_exe(g_tUART1_Handle.u8Uart_Hex_Buffer);
			...
			{   
				switch(sta)
				{
					case USMART_FUNCERR:
						//rt_kprintf("函数错误!\r\n");
                      	rt_kprintf("function error\r\n");		
						break;	
					case USMART_PARMERR:
						//rt_kprintf("参数错误!\r\n");   			
						rt_kprintf("parameters error\r\n");
						break;				
					case USMART_PARMOVER:
						//rt_kprintf("参数太多!\r\n");   			
						rt_kprintf("parameters too much!\r\n");
						break;		
					case USMART_NOFUNCFIND:
						//rt_kprintf("未找到匹配的函数!\r\n");   			
						rt_kprintf("No matching function found!\r\n");
						break;		
				}
			}
		}
//		USART_RX_STA=0;//状态寄存器清空	
		g_tUART1_Handle.u16Uart_Rx_Sta = 0;
	}
}
