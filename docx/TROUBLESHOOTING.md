...
### 追问一：之前的项目没这样调整，USB 也一直用得好，是不是分析错了？

不矛盾。大概率是之前那个项目的主频本来就不是 180MHz，而是恰好落在能让 `PLLQ` 整除出 48MHz 的值上（168MHz 是最常见的一个，很多 STM32F4 + USB 的教程/模板默认就给的是 168MHz）——只是当时没把它当成"专门为了 USB 调的"，因为那是模板一开始就给的默认值，没有一个"180MHz"的对比基准让人觉得"降频率了"。

而且这个"主 PLL 的 `Q` 输出是 `CK48M` 唯一来源"的限制，不是 F429/439 特有的坏消息——除了 F469/F479（以及走 PLLI2S 的 F412/413/423）之外，F405/407/411/417 这些常见型号都是同样的约束。所以只要之前用的是这类"经典 F4"，USB 能正常工作基本可以反推：当时的主频本来就是某个能整除出 48MHz 的"标准值"，不是巧合，是模板本来就设计对了。

### 追问二：如果不这样调整，`CK48M` 不是精确 48MHz，USB 真的会有问题吗？

有影响，但只对 USB 明显，`SDIO`/`RNG` 基本没事——这两类外设对时钟精度的敏感程度完全不同：

- **USB**：USB 2.0 全速（12Mbps）规范要求收发器时钟精度在 **±0.25%**（2500ppm）以内，因为主机和设备各用自己的晶振独立计时，靠 NRZI 编码 + 位填充做同步，时钟差一点，每个字节累积的相位误差就会让采样点偏出对方发出的电平中心。`45MHz` 相对标准 `48MHz` 偏差 `(48-45)/48 ≈ 6.25%`，是规范允许误差的**25 倍**，彻底超标。实际表现通常是：可能枚举失败、可能能枚举但传输时频繁 CRC 错误/掉线重连/吞吐量差，而且**不稳定**——换根线、换台电脑结果可能完全不同，短距离高质量连接下侥幸能用，条件差一点就出问题。
- **SDIO**：MCU 自己出时钟给 SD 卡的同步总线，不需要跟外部独立时钟对齐，时钟差一点只是总线速度按比例跟着变慢/变快，功能不受影响。
- **RNG**：对时钟精度更不敏感，只要频率在芯片规定范围内能正常起振就行。

结论：如果确定不用 USB（只用 SDIO/RNG，或都不用），这次 168MHz 的调整就是"锦上添花"，不改也不会出问题；但只要 USB OTG FS 会被真正用起来，这个偏差就不是能忽略的细节。
